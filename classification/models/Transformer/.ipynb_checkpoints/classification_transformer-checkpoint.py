...
class DocumentClassification:
    ...
    def train_model(self, show_progress_bar=True, extended_metrics=True, save_model_per_epoch=False):
        ...
        path = os.path.join(checkpoint_dir.split('config')[0]) + '/model/best/'
        ...
        for epoch in trange(self.config.num_epochs, desc="Epoch", disable=not show_progress_bar):
            ...
            os.makedirs(os.path.join(checkpoint_dir.split('config')[0]) + '/model/' + str(epoch+1), exist_ok=True) 
            ...
            metrics_df.to_csv(f"{path}/training_metrics.csv", mode='a', index=False)
            ...
            if save_model_per_epoch:
                path = os.path.join(checkpoint_dir.split('config')[0]) + f'/model/{epoch}/'
                if not os.path.exists(path):
                    os.makedirs(path)
        ...
        self.model = torch.load(os.path.join(checkpoint_dir.split('config')[0]) + f'/model/best/bert_model_{max_seq_length}.pt')
